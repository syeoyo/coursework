...
\begin{frame}{분산에너지원 (Distributed Energy Resources, DER)}
    \textbf{에너지를 사용하는 지역 또는 인근지역에서 공급하거나 생산하는 에너지로, 일정 규모 이하의 소규모 에너지 \footnotemark[1]}
    \begin{itemize}
        \item 태양광(PV) 및 풍력 터빈(WT) 발전 장치, 에너지 저장 시스템(ESS), 전기 자동차(EV) 충전기, 마이크로그리드 등을 포함
        \item \underline{소규모 발전의 형태}에 적합한 \underline{탄소중립적인 에너지}를 중심으로 생산
    \end{itemize}
...
\begin{frame}{전력시장 (Wholesale Electricity Market)}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item 공시된 가격에 따라 일정량의 전력 공급을 \textbf{하루 전에 계약} (commitment)
            \item \textbf{계약 불이행 시 페널티 발생}
        \end{itemize}
        \column{0.48\textwidth}
        \raggedright
    ...
    \begin{figure}[htb!]
        ...
        \begin{minipage}[t]{0.48\linewidth} % 동일한 너비 지정
            \centering
            \includegraphics[width=0.98\linewidth]{fig_avgprice.pdf}
            \vspace{-1em}
        ...
        \begin{minipage}[t]{0.48\linewidth} % 동일한 너비 지정
            \centering
            \includegraphics[width=0.98\linewidth]{fig_price.pdf}
            \vspace{-1em}
    ...
    \vspace{-1.8em}
    \begin{block}{}
        $\Rightarrow$ 안정적인 day-ahead commitment와 전략적인 real-time dispatching을 통한 시장참여가 중요
    \end{block}
...
\begin{frame}{분산에너지원의 통합 (Aggregation)}
    \textbf{여러 개별 분산에너지원들을 하나의 포트폴리오(Aggregated Distributed Energy Resources, ADER)로 통합하는 과정으로, 가상 발전소나 에너지 관리 시스템 등을 통해 구현}
    \begin{block}{Aggregation이 필요한 이유}
        \textcircled{\small{1}} 재생에너지 발전량의 불확실성과 \textcircled{\small{2}} 제한적인 dispatchability로 인해\\ 안정적인 day-ahead commitment를 하기 어렵고 그에 따라 페널티가 부과될 위험이 높음
    \end{block}
    ...
    \begin{itemize}
        ...
        \item 하나의 포트폴리오로서 최적화된 day-ahead commitment와 real-time trading 수행
    \end{itemize}
    \begin{figure}[h]
        \centering
...
\begin{frame}{공정성에 대한 고려}
    ...
    \begin{alertblock}{}
        ...
        \begin{itemize}
            \item 페널티 발생 위험 증가
            \item 포트폴리오의 기대 수익 감소
            \item 개별적으로 정산받을 수 있는 양 감소
        \end{itemize}
        $\Rightarrow$ 포트폴리오 내의 개별 DER들 사이 공정성이 고려되어야 함
    \end{alertblock}

    \underline{본 프로젝트에서의 공정성 정의:}
    \begin{itemize}
        \item \textbf{기여도 기반}: 포트폴리오의 페널티 방지를 위해 각 참여자가 기여한 정도가 유사해야 함
...
\begin{frame}{공정성 기반 위험 측정 (Fairness-Aware Risk Factor)}
    각 DER \(i\)의 $T$시간 동안의 시나리오$(\xi_s)$별 \textbf{총 기여도(금액)}:
    \[
    ...
    \begin{itemize}
        ...
        \item \textbf{개인의 기회비용}으로서 정의되는 marginal contribution
    \end{itemize}
    \vspace{-0.5em}
    \begin{alertblock}{Central Deviation 활용}
        ...
        \begin{itemize}
            ...
            \item 공정성은 전체 시나리오에서의 기여 편차를 지속적으로 관리해야 실질적으로 보장되므로, tail risk만 반영하는 quantile risk measure보다 deviation risk measure가 더 적합하다고 판단
        \end{itemize}
    \end{alertblock}
...
\begin{frame}{실험 세팅}
    ...
    \begin{minipage}[t]{0.45\linewidth}
        New York ISO의 데이터 기반 실험 진행 \footnotemark[3]
        \begin{itemize}
            \item 태양광 기반 DER 12개를 선정
    ...
    \begin{minipage}[t]{0.52\linewidth}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=0.89\linewidth]{fig_randgen.pdf}
            \vspace{-1em}
            \caption{시나리오별 DER 0의 발전량 예시}
        \end{figure}
        \vspace{-2em}
        \begin{figure}
            \centering
            \includegraphics[width=0.89\linewidth]{fig_randprice.pdf}
            \vspace{-1em}
...
\begin{frame}{추후 계획}
    \begin{enumerate}
        \item VSS 분석 : deterministic 해와 stochastic 해의 가치 차이를 정량적으로 비교하여 불확실성 고려의 필요성 검증
              \vspace{0.5em}
        \item Risk-Neutral 모델과의 비교
              \begin{itemize} \normalsize
                  \item 동일한 조건 하에서 risk-neutral vs risk-averse 해 비교
                  \item 공정성 리스크 도입 전후의 DER 간 기여 편차 변화 확인
              \end{itemize} \vspace{0.5em}
        \item Risk-Averseness에 따른 Trade-off 및 민감도 분석
              \begin{itemize} \normalsize
...
\setbeamertemplate{frametitle continuation}{}
\begin{frame}[noframenumbering]
    \frametitle{Appendix. Risk-Neutral 수리 모델 (1)}
    \vspace{-1em}
    \footnotesize
    \begin{block}{}
        \vspace{-1em}
    ...
    \small
    \begin{itemize}
        \item[(1a)] 포트폴리오의 하루전시장 + 실시간시장 수익의 기대값을 최대화
...
\setbeamertemplate{frametitle continuation}{}
\begin{frame}[noframenumbering]
    \frametitle{Appendix. Risk-Neutral 수리 모델 (2)}
    \vspace{-1em}
    \begin{block}{}
        \vspace{-0.7em}
        \begin{subequations}
    ...
    \begin{itemize}
        ...
        \item[(1k)] 포트폴리오의 실시간시장에서의 판매와 미이행 페널티 제출은 동시에 발생할 수 없음
...
\setbeamertemplate{frametitle continuation}{}
\begin{frame}[noframenumbering]
    \frametitle{Appendix. Risk-Averse 수리 모델}
    \vspace{-1em}
    \begin{block}{}
        \vspace{-1em}
        \begin{subequations}
    ...
    \small
    \begin{itemize}
        \item[(2a)] 포트폴리오의 기대 수익을 최대화하되, 기여도 편차에 따른 공정성 리스크를 페널티로 부여
        \item[(2b)] DER $i$의 총 기여 금액은 포트폴리오의 페널티 상쇄를 위해 제공한 에너지를 기회비용으로 책정
